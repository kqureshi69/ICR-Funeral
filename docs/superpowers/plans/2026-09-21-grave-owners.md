  ...
  <div class="modal-backdrop" id="ownerModal" hidden>
    <div class="modal">
      ...
      <form id="ownerForm">
        ...
        <label>Name
          <input type="text" id="ownerName" required placeholder="e.g. Dana Example" />
        </label>
        <label>Contact
...
Expected: `owner_name` and `owner_contact` absent from `graves`; **9 owners**; `Dana Example` with contact `555-012-3456` holding 2 plots; `unassigned: 3`.
...
2. `Dana Example`'s discarded contact `555-012-345` is gone from the live data but present in the Step 1 backup if it was the correct number.
