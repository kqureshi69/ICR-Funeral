...
|---|---|
| Same person entered twice, contacts diverge | `Dana Example` / `555-012-345` **and** `Dana Example` / `555-012-3456` |
...
2. **Duplicate merge keeps the first contact.** `Dana Example` becomes one owner
   with `555-012-3456`; `555-012-345` is discarded (the discarded value is
...
(`next-of-kin: 555-0177`, `acosta.fam@example.com 555-098-7654`); parsing them
...
**Expected result on the live database:** 9 owners; `Dana Example` merged onto
`555-012-3456`; 3 graves left unassigned. `Estate of John Whitfield` and
...
count ("Dana Example still holds 2 plots. Reassign them first."), consistent with
